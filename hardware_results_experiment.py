#!/usr/bin/env python3
"""Run the frozen quantum-kernel hardware results experiment.
...
generates results tables and figures.
...
# Pinned to preserve the completed campaign's intervals across file renaming.
BOOTSTRAP_RANDOM_SEED = 558_904_967
...
RESULT_ROOT = Path("hardware_results/results")
...
def write_pre_hardware_summary() -> None:
    campaign = load_or_create_campaign()
    lines = [
        "SRP2026 FROZEN QUANTUM-KERNEL EXPERIMENT: PRE-HARDWARE RESULTS",
        "=" * 62,
        ...
        "claim. Final results will be written to analysis/results.txt.",
        "",
        "PROTOCOL",
        "--------",
...
def _batch_job_snapshots(
    ...
    results campaign submits jobs in batches, so the Runtime batch-list API
...
def _write_results_report(
    ...
    lines = [
        "SRP2026 QUANTUM-KERNEL HARDWARE RESULTS",
        "=" * 57,
        f"Generated (UTC): {utc_now()}",
        "",
        "SCOPE",
    ...
    lines.extend(
        [
            ...
            "tables/*.tex: captioned LaTeX results tables",
    ...
    atomic_write_text(ANALYSIS_DIR / "results.txt", "\n".join(lines))
...
def _results_latex_tables(
    ...
    """Create the results tables as captioned LaTeX fragments."""
...
def _results_figures(metric_rows: Sequence[Mapping[str, Any]]) -> None:
    frame = pd.DataFrame(metric_rows)
...
def analyze_results() -> None:
    ...
    save_campaign(campaign)
    print(f"Results analysis written to {ANALYSIS_DIR}", flush=True)
